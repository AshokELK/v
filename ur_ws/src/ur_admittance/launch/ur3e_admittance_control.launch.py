...
def generate_launch_description():
    pkg_ur_admittance = FindPackageShare('ur_admittance').find('ur_admittance')
    
    # Include the UR3e with gripper spawn launch file
    spawn_robot = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            os.path.join(pkg_ur_admittance, 'launch', 'spawn_ur3e_gripper.launch.py')
        ]),
        launch_arguments={
            'robot_ip': '',
            'ur_type': 'ur3e',
            ...
            'sim_gazebo': 'true',
            'simulation_controllers': os.path.join(pkg_ur_admittance, 'config', 'ur3e_controllers_gripper.yaml'),
            'description_package': 'ur_admittance',
    ...
    rviz_config = PathJoinSubstitution([pkg_ur_admittance, 'rviz', 'ur3e_admittance.rviz'])
    rviz = Node(
        package='rviz2',
        executable='rviz2',
        name='rviz2',
